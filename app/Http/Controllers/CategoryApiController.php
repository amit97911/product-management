...
class CategoryApiController extends Controller
{
    ...
    {
        $zeroCats = LevelZeroCategory::select('category_id')->pluck('category_id')->toArray();
        $oneCats = LevelOneCategory::select('category_id')->pluck('category_id')->toArray();
        $twoCats = LevelTwoCategory::select('category_id')->pluck('category_id')->toArray();
        $threeCats = LevelThreeCategory::select('category_id')->pluck('category_id')->toArray();
        $now = now();
        ...
        if ($response['status'] == 'success') {
            ...
            foreach ($categories as $category0) {
                if (! in_array($category0['id'], $zeroCats)) {
                ...
                foreach ($category0['childrenTree'] as $category1) {
                    if (! in_array($category1['id'], $oneCats)) {
                    ...
                    foreach ($category1['childrenTree'] as $category2) {
                        if (! in_array($category2['id'], $twoCats)) {
                        ...
                        foreach ($category2['childrenTree'] as $category3) {
                            if (! in_array($category3['id'], $threeCats)) {
